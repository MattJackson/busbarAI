...
description: "Swap a pool's selection strategy with one field: weighted, cheapest, fastest, least-busy natively, or your own logic via an HTTP webhook or a compiled socket hook."
---
...
Routing is the first of Busbar's **Hooks**: a programmable request path. At a small number of points in the request lifecycle, an operator-supplied policy inspects the request and returns a decision, via one of the hook transports: native (built-in), webhook (HTTP, any language), or socket (a compiled binary on a local Unix domain socket, about 8 microseconds per decision). The embedded Rhai script transport is deprecated. Routing is hook #1: it decides selection order. The same transport machinery (timeout, fallback, transparency header) carries every hook, so a broken or slow policy never blocks or fails a request.
...
- [External policies](#external-policies)
  ...
  - [script (Rhai, deprecated)](#script-rhai-deprecated)
...
- `reject` wins over `order` and `abstain` if sent together, and the verb is **fail-closed**: any `reject` value except an explicit `false` (or `null`) is a rejection, even a malformed one — a mis-typed detail degrades to the defaults, never to "silently route the request". Both fields are optional: a bare `{"reject":{}}` rejects with the defaults (403, a generic message). `status` is clamped to 400–499 — a hook cannot mint a success, a redirect, or a 5xx — and `message` is sanitized (control characters stripped, length capped) before it reaches the client error body. The status also picks the dialect error type the SDK sees: 401 → authentication, 403 → permission, 404 → not-found, 408 → timeout, 429 → rate-limit, anything else → invalid-request. A rejection is a deliberate decision, not a failure: `on_error` does not apply, and each one increments `busbar_route_policy_rejections_total` (labels: policy, pool, status). Combined with `send_prompt` this is the PII-screen primitive: a hook that sees content can stop a request before it leaves your network.
...
**Security.** The path is operator config on the local filesystem: no port, no TLS, no SSRF surface — the decision cannot leave the machine. Access control is filesystem permissions on the socket file (e.g. `0600` under a shared service user). Replies are read under the same 64 KiB cap and depth-guarded JSON parse as the webhook.

**Portability.** Unix-only (macOS/Linux, any architecture). On other platforms `route: socket` is a startup error pointing at `route: webhook`.

**Wire compatibility.** The request and reply schemas are byte-identical to the webhook's, so a policy graduates from a webhook prototype to a compiled socket binary without changing its logic. A complete Rust hook (about a hundred lines, stdlib + serde) ships in the repo under `examples/smart-router/rust-hook/`.

### script (Rhai, deprecated)

`route: script`

> **Deprecated as of 1.2.1.** The embedded interpreter costs ~20x more per decision than a compiled socket hook running the same logic, and an in-process engine is a weaker isolation story than a separate process. It still works behind the `script-policy` cargo feature but logs a deprecation warning at startup; migrate to `route: socket` or `route: webhook`.
...
|---|---|---|---|
| `route` | string | `weighted` | Routing transport: `weighted`, `native` (or a native shorthand: `cheapest`/`fastest`/`least_busy`/`usage`), `webhook`, `socket`, or `script` (deprecated). `weighted` (default / absent) runs SWRR with zero added cost. |
| `policy` | object | none | Transport configuration block. Required for `native`, `webhook`, `socket`, and `script`; inert for `weighted`. |
...
Webhook and socket misconfiguration are hard startup errors. A misconfigured `native` or `script` policy never fails the boot; it degrades to weighted SWRR (a script fallback logs a warning), because routing must never take the gateway down.
...
| `route: socket` + no `policy.socket` | **Startup error** |
| `route: socket` + relative `policy.socket` path | **Startup error** (a relative path depends on Busbar's working directory) |
| `route: socket` on a non-unix platform | **Startup error** (use `route: webhook` there) |
