...
Enforcement happens **before the request is forwarded**: the budget check and the charge are a
single atomic operation, and a request that would exceed its cap or RPM limit is rejected at
the gate, not reconciled after the bill. It's off by default; turn it on when you need budgets,
rate limits, spend visibility, or access control. Each capability below has two views; read **What it does** for the why, flip to
