...
export const hooks: Hook[] = [
  ...
  headroom:
    ...
    detail: [
      ...
      'Measured, reproducibly — the rig ships in the repo. On an 11&nbsp;KB noisy tool-log history, one request at a time:<table class="hook-bench"><thead><tr><th></th><th>input tokens</th><th>added latency (p50)</th></tr></thead><tbody><tr><td>without Headroom</td><td>2,832</td><td>—</td></tr><tr><td><strong>Headroom on Busbar</strong> (socket gate)</td><td><strong>1,422</strong> (−50%)</td><td><strong>~620&nbsp;µs</strong></td></tr><tr><td>Headroom’s own HTTP proxy <span class="bench-note">(their published production median)</span></td><td>—</td><td>52&nbsp;ms</td></tr></tbody></table>The 620&nbsp;µs is the with/without-hook delta, so the benchmark harness’s round-trip floor cancels and what’s left is the compression plus its socket round trip, on top of Busbar’s own tens-of-µs overhead. On a two-second inference call that’s <strong>0.03%</strong> of the request. Savings are content-dependent (noisy tool/log/RAG history compresses 40–90%); short conversational chats abstain and pass through byte-identical, and if the hook is ever slow, wrong, or down the request proceeds with its original body. The 52&nbsp;ms proxy figure is negligible against inference too, as Headroom rightly notes — the point isn’t a race, it’s that a co-located gate removes the network hop a separate proxy can’t.',
    ],
    spotlight: {
      project: 'Headroom',
