...
export const hooks: Hook[] = [
  {
    ...
    install: {
      note: 'The hook is a small Rust binary you run yourself; Busbar connects to it over a Unix socket. You own its lifecycle — Busbar never spawns it, lazy-connects, and reconnects across restarts, so start order doesn’t matter.<br><br><strong>1. Get the binary.</strong> Grab the prebuilt one for your platform from the <a href="https://github.com/GetBusbar/Hooks/releases/latest" target="_blank" rel="noopener">latest release</a>:<div class="os-tabs"><input type="radio" name="os" id="os-lx" checked><input type="radio" name="os" id="os-la"><input type="radio" name="os" id="os-mac"><div class="os-tab-labels"><label for="os-lx">Linux x86_64</label><label for="os-la">Linux arm64</label><label for="os-mac">macOS · Apple Silicon</label></div><pre class="install-code os-panel p-lx"><code>curl -fsSL -o headroom-hook \\\n  https://github.com/GetBusbar/Hooks/releases/latest/download/headroom-hook-linux-x86_64\nchmod +x headroom-hook</code></pre><pre class="install-code os-panel p-la"><code>curl -fsSL -o headroom-hook \\\n  https://github.com/GetBusbar/Hooks/releases/latest/download/headroom-hook-linux-aarch64\nchmod +x headroom-hook</code></pre><pre class="install-code os-panel p-mac"><code>curl -fsSL -o headroom-hook \\\n  https://github.com/GetBusbar/Hooks/releases/latest/download/headroom-hook-macos-arm64\nchmod +x headroom-hook</code></pre></div>Or build from source (needs <a href="https://rustup.rs" target="_blank" rel="noopener">Rust</a>; grabs just this hook): <code>git clone --filter=blob:none --sparse https://github.com/GetBusbar/Hooks &amp;&amp; cd Hooks &amp;&amp; git sparse-checkout set Headroom &amp;&amp; cargo build --release --manifest-path Headroom/Cargo.toml</code><br><br><strong>2. Run it</strong> on a socket:<br><pre class="install-code"><code>HEADROOM_SOCKET=/run/busbar/headroom.sock ./headroom-hook</code></pre><strong>3. Point Busbar at it</strong>, or register it live over the admin API:',
      code: `hooks:
  headroom:
    kind: gate
