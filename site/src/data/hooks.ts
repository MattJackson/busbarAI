...
export const hooks: Hook[] = [
  ...
  headroom:
    ...
    on_error: nothing   # a broken compressor never touches a request`,
      foot: 'Unix socket (Linux + macOS, any arch). On Windows, run the hook on a Linux host or container and register it as a <code>webhook:</code> sidecar instead of <code>socket:</code> — same wire, over HTTP.',
    },
    ...
    detail: [
      ...
      'Measured, reproducibly (the rig ships in the repo): on an 11KB noisy tool-log history the hook cuts input tokens from 2,832 to 1,422 per request, a 50% reduction confirmed at the upstream. Measured one request at a time, the hook adds about 620 microseconds (µs) to a request — the with-hook minus without-hook delta, so the harness floor cancels and what remains is the compression plus its socket round trip, on top of Busbar’s own tens-of-µs overhead. Total added latency stays well under a millisecond; on a two-second inference call that is 0.03% of the request. For comparison, Headroom’s own HTTP proxy reports a 52ms median overhead in production (negligible against inference, as they note); running the same core as a co-located socket gate removes the network hop, so the added latency is sub-millisecond. Short conversational chats abstain and pass through byte-identical, and if the hook is ever slow, wrong, or down the request proceeds with its original body.',
    ],
    spotlight: {
      project: 'Headroom',
