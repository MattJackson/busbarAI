...
export const hooks: Hook[] = [
  {
    ...
    install: {
      note: 'The hook is a small binary you run alongside Busbar; Busbar connects to it over a Unix socket. You own its lifecycle — Busbar never spawns it, lazy-connects, and reconnects across restarts, so start order doesn’t matter.<br><br><strong>Docker — <code>docker compose up</code>.</strong> Two tiny images, one shared socket. Grab the <a href="https://github.com/GetBusbar/headroom-hook/blob/main/docker-compose.yml" target="_blank" rel="noopener">compose file</a>, drop your Busbar <code>config.yaml</code> next to it, and <code>docker compose up</code> brings Busbar and the compression hook up together — Headroom is on every request.<br><br><strong>Or run the binary yourself. 1. Get it</strong> for your platform from the <a href="https://github.com/GetBusbar/headroom-hook/releases/latest" target="_blank" rel="noopener">latest release</a>:<div class="os-tabs"><input type="radio" name="os" id="os-lx" checked><input type="radio" name="os" id="os-la"><input type="radio" name="os" id="os-mac"><div class="os-tab-labels"><label for="os-lx">Linux x86_64</label><label for="os-la">Linux arm64</label><label for="os-mac">macOS · Apple Silicon</label></div><pre class="install-code os-panel p-lx"><code>curl -fsSL -o headroom-hook \\\n  https://github.com/GetBusbar/headroom-hook/releases/latest/download/headroom-hook-linux-x86_64\nchmod +x headroom-hook</code></pre><pre class="install-code os-panel p-la"><code>curl -fsSL -o headroom-hook \\\n  https://github.com/GetBusbar/headroom-hook/releases/latest/download/headroom-hook-linux-aarch64\nchmod +x headroom-hook</code></pre><pre class="install-code os-panel p-mac"><code>curl -fsSL -o headroom-hook \\\n  https://github.com/GetBusbar/headroom-hook/releases/latest/download/headroom-hook-macos-arm64\nchmod +x headroom-hook</code></pre></div>Or build from source (needs <a href="https://rustup.rs" target="_blank" rel="noopener">Rust</a>; grabs just this hook): <code>git clone https://github.com/GetBusbar/headroom-hook &amp;&amp; cd headroom-hook &amp;&amp; cargo build --release</code><br><br><strong>2. Run it</strong> on a socket:<br><pre class="install-code"><code>HEADROOM_SOCKET=/tmp/headroom.sock ./headroom-hook</code></pre><strong>3. Point Busbar at it</strong>, or register it live over the admin API:',
      code: `hooks:
  headroom:
    kind: gate
