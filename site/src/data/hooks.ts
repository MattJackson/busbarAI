...
export interface Hook {
  ...
  hookRepo?: string;
  // Docker Hub repo (namespace/name). When set, the detail page shows a Docker Hub badge with the
  // live pull count next to the GitHub link.
  dockerRepo?: string;
  install?: { note: string; code?: string; foot?: string };
...
export const hooks: Hook[] = [
  {
    ...
    body: 'A rewrite gate that trims and compacts a request’s context before it ships, so a long conversation stays under the window without your app rebuilding the prompt. Written once, it works against every protocol and provider Busbar speaks. Measured: about 50% fewer input tokens on noisy histories for sub-millisecond overhead; short chats pass through untouched.',
    repo: 'https://github.com/GetBusbar/headroom-hook',
    status: 'Available',
    href: '/hooks/headroom',
    hookRepo: 'https://github.com/GetBusbar/headroom-hook',
    dockerRepo: 'getbusbar/headroom-hook',
    install: {
      note: 'The hook is a small binary you run alongside Busbar; Busbar connects to it over a Unix socket. You own its lifecycle — Busbar never spawns it; it lazy-connects and reconnects across restarts, so start order doesn’t matter.<br><br><strong>Docker (easiest).</strong> Two tiny images, one shared socket — copy, paste, running:<br><pre class="install-code"><code>curl -fsSL -O https://raw.githubusercontent.com/GetBusbar/headroom-hook/main/docker-compose.yml\ncurl -fsSL -o config.yaml https://raw.githubusercontent.com/GetBusbar/headroom-hook/main/config.example.yaml\nexport ANTHROPIC_KEY=sk-ant-...   # or edit config.yaml for your provider\ndocker compose up</code></pre>Busbar (<code>:8080</code>) and the compression hook come up together — Headroom is on every request. Edit <code>config.yaml</code> for your own pools. The images are <code>getbusbar/headroom-hook</code> and <code>getbusbar/busbar</code>.<br><br><strong>Manual — 1. Get the binary</strong> for your platform from the <a href="https://github.com/GetBusbar/headroom-hook/releases/latest" target="_blank" rel="noopener">latest release</a>:<div class="os-tabs"><input type="radio" name="os" id="os-lx" checked><input type="radio" name="os" id="os-la"><input type="radio" name="os" id="os-mac"><div class="os-tab-labels"><label for="os-lx">Linux x86_64</label><label for="os-la">Linux arm64</label><label for="os-mac">macOS · Apple Silicon</label></div><pre class="install-code os-panel p-lx"><code>curl -fsSL -o headroom-hook \\\n  https://github.com/GetBusbar/headroom-hook/releases/latest/download/headroom-hook-linux-x86_64\nchmod +x headroom-hook</code></pre><pre class="install-code os-panel p-la"><code>curl -fsSL -o headroom-hook \\\n  https://github.com/GetBusbar/headroom-hook/releases/latest/download/headroom-hook-linux-aarch64\nchmod +x headroom-hook</code></pre><pre class="install-code os-panel p-mac"><code>curl -fsSL -o headroom-hook \\\n  https://github.com/GetBusbar/headroom-hook/releases/latest/download/headroom-hook-macos-arm64\nchmod +x headroom-hook</code></pre></div>Or build from source (needs <a href="https://rustup.rs" target="_blank" rel="noopener">Rust</a>): <code>git clone https://github.com/GetBusbar/headroom-hook &amp;&amp; cd headroom-hook &amp;&amp; cargo build --release</code><br><br><strong>2. Run it</strong> on a socket:<br><pre class="install-code"><code>HEADROOM_SOCKET=/tmp/headroom.sock ./headroom-hook</code></pre><strong>3. Point Busbar at it</strong>, or register it live over the admin API:',
      code: `hooks:
  headroom:
    kind: gate
    ...
    on_error: nothing   # a broken compressor never touches a request`,
      foot: '<strong>Dashboard.</strong> The hook emits Headroom’s own <code>headroom_*</code> Prometheus metrics, and Busbar re-exposes them on <code>/metrics/hooks</code> — so <strong>your existing Headroom Grafana dashboard just works, unchanged.</strong> Nothing to re-point, nothing to rebuild. A ready-to-import dashboard also ships in the <a href="https://github.com/GetBusbar/headroom-hook/tree/main/grafana" target="_blank" rel="noopener">hook repo</a>.<br><br><strong>Platforms.</strong> The hook speaks a Unix socket, so it runs on Linux and macOS (any arch). There is no native Windows build — run Busbar and the hook together inside WSL2 or a Linux container, where the socket works normally.',
    },
    ...
    detail: [
      ...
      'Measured the way Busbar measures itself — from Busbar’s own clock. Busbar reports its internal processing time (total minus the upstream round-trip) in a <code>Server-Timing: busbar;dur</code> header, and the Headroom gate runs synchronously inside that window, so this is exactly what the hook adds, floor-free. <code>busbar;dur</code> in µs, concurrency 1, 1,000 requests, 11&nbsp;KB noisy tool-log history:<table class="hook-bench"><thead><tr><th></th><th>p50</th><th>p90</th><th>p99</th></tr></thead><tbody><tr><td>Busbar alone</td><td>22&nbsp;µs</td><td>25&nbsp;µs</td><td>30&nbsp;µs</td></tr><tr><td>Busbar + Headroom</td><td>569&nbsp;µs</td><td>601&nbsp;µs</td><td>634&nbsp;µs</td></tr><tr><td>Headroom’s added cost</td><td>547&nbsp;µs</td><td>576&nbsp;µs</td><td>604&nbsp;µs</td></tr></tbody></table>Two things stand out: Busbar itself is <strong>tens of µs</strong> (the control plane isn’t where your latency goes), and Headroom adds <strong>547&nbsp;µs</strong> with a tight tail (p99 only ~1.1× p50 — both are single Rust binaries, no garbage collector pausing the path). On a two-second inference call that’s <strong>0.03%</strong> of the request.',
      'This isn’t a compression benchmark — how well Headroom compresses is the <a href="https://headroomlabs-ai.github.io/headroom/" target="_blank" rel="noopener">Headroom project’s</a> craft, and it reports higher ratios than the ~50% seen here (66–94% on some content), with more to gain from tuning. What we checked is that the plumbing is sound: the mock upstream tallies the tokens it actually received, so this really shipped to the provider — an 11&nbsp;KB noisy tool-log history goes <strong>2,832 → 1,422 input tokens</strong>. Short conversational chats abstain and pass through byte-identical (100% abstain over 100 short chats), and if the hook is ever slow, wrong, or down the request proceeds with its original body.',
      'For scale: Headroom’s own HTTP proxy reports a <strong>52&nbsp;ms median</strong> overhead from production telemetry (50,000+ sessions), which — as they rightly note — is negligible against multi-second inference. The same compression core, run as a Busbar gate, measures in the hundreds of microseconds on Busbar’s clock. Both are small next to the model call; we make no claim about how their proxy is deployed. Headroom does the compression; Busbar just puts it in front of every model you call.',
    ],
    spotlight: {
      project: 'Headroom',
