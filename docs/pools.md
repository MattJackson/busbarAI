...
Set `route:` to something other than `weighted` and a **routing policy** decides the order instead (cheapest, fastest, least busy, or your own webhook/Rhai logic). The policy runs once per request, before the failover loop.
...
pools:
  budget:
    route: cheapest
    members:
      - { target: gpt-4o-mini,  cost_per_mtok: 0.15 }
      - { target: claude-haiku, cost_per_mtok: 0.25 }
      - { target: gpt-4o,       cost_per_mtok: 2.50 }   # last resort
```

### Lowest latency or least loaded

```yaml
pools:
  realtime:
    route: fastest          # or: least_busy
    members:
      - { target: gpt-4o }
      - { target: claude-sonnet }
```

### Tiered: prefer primaries, spill to a cheap overflow
...
pools:
  smart:
    route: least_busy
    members:
      - { target: gpt-4o,        tier: primary }
      - { target: claude-sonnet, tier: primary }
      - { target: gpt-4o-mini,   tier: overflow, cost_per_mtok: 0.15 }
```
...
### Your own routing logic (webhook or Rhai script)
...
pools:
  custom:
    ...
    policy:
      ...
      timeout_ms: 50
      on_error: weighted            # weighted | reject | first
    members:
      - { target: gpt-4o }
      - { target: claude-sonnet }
```

See the [Routing guide](/routing/) for the full policy contract and the signals each policy receives, and [Reliability](/reliability/) for how the breaker and failover behave once a policy has chosen an order.
