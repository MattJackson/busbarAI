...
- **A much larger provider catalog out of the box** — by choice, not limitation. Busbar ships a *vetted* catalog, not a scraped one: each provider's error-code mappings are hand-verified because they drive the breaker's fault attribution, so we'd rather stand behind a curated set than pad a count that goes stale. Any provider that speaks one of the six protocols is a few lines of YAML you add yourself ([Adding a provider](/providers/)), so "not in the catalog" rarely means "can't use it." The genuine gap is an exotic provider whose wire format is *none* of the six and not compatible with any — that needs a new protocol reader. And most teams run a handful of providers, not a hundred.
...
- **A hosted dashboard product** — *coming soon for Busbar.* Today, observability runs through Prometheus `/metrics`, OTLP traces, and the admin API; a hosted dashboard is on the roadmap.
