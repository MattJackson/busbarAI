...
| `on_error` | string | `weighted` | Fallback when a gate times out / errors / saturates: `nothing` (do not participate — a failing non-routing gate can never displace another gate's verdict), `weighted` (the ordering floor; same behavior, the name for ordering gates), `reject` (fail closed — security gates set this), `first`, or the NAME of a fallback hook (a chain, proven terminating at boot). A gate's deliberate `reject` reply is a decision, not a failure — `on_error` never applies to it. |
