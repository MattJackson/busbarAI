...
The budget above bounds the whole request. It does nothing for a provider that **hangs**, accepting the connection and never returning headers, because the hang quietly eats the entire budget on one member. `attempt_timeout_ms` (settable on a model, overridable per pool member) caps a single attempt's time to response headers: on expiry the attempt is recorded as a transient breaker failure and the request hops to the next member immediately. The per-pool override means the same model can carry a 10s cap in a batch pool and a 50ms cap in a latency-critical one. The cap never cuts a stream that has started answering (it covers connect + headers only), and it is always floored by the request's remaining `failover.timeout_secs`. Full semantics and examples in the [configuration reference](https://getbusbar.com/docs/configuration/#per-attempt-timeouts-attempt_timeout_ms).
