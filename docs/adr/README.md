...
|---|---|---|
| [0001](0001-weighted-selection.md) | Smooth weighted round-robin (SWRR) selection | `src/store/mod.rs` |
| [0002](0002-circuit-breaker.md) | Circuit breaker: disposition taxonomy & recovery | `src/breaker.rs`, `src/store/mod.rs`, `src/proxy/engine/mod.rs` |
| [0005](0005-ir-fidelity.md) | Superset IR & translation fidelity | `src/ir/mod.rs`, `src/proto/` |
...
- `ADR-0008` — the string-keyed `ProtocolRegistry` (`src/proto/mod.rs`, `src/config/mod.rs`).
- `ADR-0009` — the durable governance `Store` seam / SqliteStore (`src/governance/mod.rs`, `src/config/mod.rs`).
