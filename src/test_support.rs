...
async fn mock_handler(
    ...
    let (status, body) = match response {
        MockResponse::Ok { status, body } => (status, body.to_string()),
        ...
        MockResponse::Auth { status } => {
            (status, serde_json::json!({ "error": "Unauthorized" }).to_string())
        }
        MockResponse::ServerError { status, body } => (status, body.to_string()),
    };
